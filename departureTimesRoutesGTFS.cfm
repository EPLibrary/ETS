...
description="Accepts FROM and TO stop IDs, and a datetime and outputs a table with relevant stops at that stop to the destination">
	...
	<cfloop query="DepartureTimes">
		...
		<tr data-tripid="#trip_id#" data-sequence="#stop_sequence#">
			<td class="tN"><cfif trip_headsign NEQ 1 AND route_id NEQ Left(stop_headsign, len(route_id))>#route_id# </cfif><cfif len(stop_headsign)>#stop_headsign#<cfelseif len(trip_headsign) AND trip_headsign NEQ 1>#trip_headsign#<cfelse>#route_short_name# #route_long_name#</cfif></td>
