...
CREATE TABLE vsd.ETS_stop_times (
	stimeID INT IDENTITY PRIMARY KEY,
	...
	shape_dist_traveled INT NULL, --theoretically could be a float, but is only ever int
)
...
CREATE NONCLUSTERED INDEX ix_ETS_STIME_stop_sequence_trip_id ON vsd.ETS_stop_times (stop_sequence) INCLUDE (trip_id)
-- This index slows imports down by another ten seconds or so but helps with MotherQuery (which I'm not using at the moment)
CREATE NONCLUSTERED INDEX ix_ETS_STIME_trip_id_stop_id_stop_sequence ON vsd.ETS_stop_times (trip_id,stop_id) INCLUDE (stop_sequence)
