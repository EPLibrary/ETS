...
<cfquery name="routeStops" dbtype="ODBC" datasource="ETSRead">
	SELECT DISTINCT sdt.stop_id, stop_name, stop_lat, stop_lon FROM dbo.#dbprefix#_trip_stop_datetimes sdt
	JOIN dbo.#dbprefix#_stops s ON s.stop_id=sdt.stop_id
	...
	AND stop_sequence > (SELECT TOP 1 stop_sequence FROM dbo.#dbprefix#_stop_times WHERE trip_id = sdt.trip_id AND stop_id=#url.routeFrom#)
